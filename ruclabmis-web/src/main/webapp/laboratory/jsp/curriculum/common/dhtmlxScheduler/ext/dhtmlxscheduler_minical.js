...
scheduler.renderCalendar = function (a, b, c) {
    var d = null, f = a.date || new Date;
    typeof f == "string" && (f = this.templates.api_date(f));
    if (b)d = this._render_calendar(b.parentNode, f, a, b), scheduler.unmarkCalendar(d); else {
        var e = a.container, h = a.position;
        typeof e == "string" && (e = document.getElementById(e));
        typeof h == "string" && (h = document.getElementById(h));
        if (h && typeof h.left == "undefined")var k = getOffset(h), h = {top: k.top + h.offsetHeight, left: k.left};
        e || (e = scheduler._get_def_cont(h));
        d = this._render_calendar(e, f, a);
        d.onclick =
            function (a) {
    ...
    if (scheduler.config.minicalendar.mark_events)for (var j =
        ...
        if (!s || s(g.id, g)) {
            ...
            for (i = scheduler.date.date_part(new Date(i.valueOf())); i < g.end_date;)if (this.markCalendar(d, i, "dhx_year_event"), i = this.date.add(i, 1, "day"), i.valueOf() >= n.valueOf())break
        }
    }
...
scheduler._get_def_cont = function (a) {
    if (!this._def_count)this._def_count = document.createElement("DIV"), this._def_count.className = "dhx_minical_popup", this._def_count.onclick = function (a) {
...
scheduler._locateCalendar = function (a, b) {
    var c = a.childNodes[2].childNodes[0];
    typeof b == "string" && (b = scheduler.templates.api_date(b));
    var d = a.week_start + b.getDate() - 1;
    return c.rows[Math.floor(d / 7)].cells[d % 7].firstChild
};
scheduler.markCalendar = function (a, b, c) {
    this._locateCalendar(a, b).className += " " + c
};
...
scheduler._week_template = function (a) {
    for (var b = a || 250, c = 0, d = document.createElement("div"), f = this.date.week_start(new Date), e = 0; e < 7; e++)this._cols[e] = Math.floor(b / (7 - e)), this._render_x_header(e, c, f, d), f = this.date.add(f, 1, "day"), b -= this._cols[e], c += this._cols[e];
...
scheduler._render_calendar = function (a, b, c, d) {
    ...
    var b = this.date.month_start(b), p = this._week_template(a.offsetWidth - 1), g;
    ...
    if (c.navigation)for (var i = function (a, b) {
    ...
    }, w = ["dhx_cal_prev_button", "dhx_cal_next_button"], x = ["left:1px;top:2px;position:absolute;",
        "left:auto; right:1px;top:2px;position:absolute;"], y = [-1, 1], z = function (a) {
        return function () {
            if (c.sync)for (var b = scheduler._synced_minicalendars, d = 0; d < b.length; d++)i(b[d], a); else i(g, a)
        }
    }, o = 0; o < 2; o++) {
    ...
    g._date = new Date(b);
    g.week_start = (b.getDay() - (this.config.start_on_monday ? 1 : 0) + 7) % 7;
    var A = this.date.week_start(b);
    this._reset_month_scale(g.childNodes[2],
        b, A);
    for (var m = g.childNodes[2].firstChild.rows, r = m.length; r < 6; r++) {
        var v = m[m.length - 1];
        m[0].parentNode.appendChild(v.cloneNode(!0));
        for (var t = parseInt(v.childNodes[v.childNodes.length - 1].childNodes[0].innerHTML), t = t < 10 ? t : 0, u = 0; u < m[r].childNodes.length; u++)m[r].childNodes[u].className = "dhx_after", m[r].childNodes[u].childNodes[0].innerHTML = scheduler.date.to_fixed(++t)
    }
    d || a.appendChild(g);
...
scheduler.destroyCalendar = function (a, b) {
    if (!a && this._def_count && this._def_count.firstChild && (b || (new Date).valueOf() - this._def_count._created.valueOf() > 500))a = this._def_count.firstChild;
    if (a && (a.onclick = null, a.innerHTML = "", a.parentNode && a.parentNode.removeChild(a), this._def_count))this._def_count.style.top = "-1000px"
};
...
scheduler.form_blocks.calendar_time = {render: function () {
    var a = "<input class='dhx_readonly' type='text' readonly='true'>", b = scheduler.config, c = this.date.date_part(new Date), d = 1440, f = 0;
    ...
    var f = a.getElementsByTagName("input"), e = a.getElementsByTagName("select"), h = function (a, b, c) {
        a.onclick = function () {
            ...
            scheduler.renderCalendar({position: a, date: new Date(this._date), navigation: !0, handler: function (b) {
                ...
                a._date = new Date(b);
                scheduler.destroyCalendar();
                scheduler.config.event_duration && scheduler.config.auto_end_date && c == 0 && l()
            }})
        }
    };
    if (scheduler.config.full_day) {
        ...
        var j = a.previousSibling.getElementsByTagName("input")[0], n = scheduler.date.time_part(c.start_date) ==
            0 && scheduler.date.time_part(c.end_date) == 0;
        j.checked = n;
        ...
        j.onclick = function () {
            if (j.checked == !0) {
                ...
                if (+g == +h || +g >= +h && (c.end_date.getHours() != 0 || c.end_date.getMinutes() != 0))g = scheduler.date.add(g, 1, "day")
            }
...
scheduler.linkCalendar = function (a, b) {
    var c = function () {
        var c = scheduler._date, f = new Date(c.valueOf());
        b && (f = b(f));
        f.setDate(1);
        scheduler.updateCalendar(a, f);
        return!0
    };
...
scheduler._markCalendarCurrentDate = function (a) {
    ...
    if (c == "day" || this._props && this._props[c])d.valueOf() <= b.valueOf() && f > b && scheduler.markCalendar(a, b, "dhx_calendar_click"); else if (c == "week")for (var e = scheduler.date.week_start(new Date(b.valueOf())), h = 0; h < 7; h++)d.valueOf() <= e.valueOf() && f > e && scheduler.markCalendar(a, e, "dhx_calendar_click"), e = scheduler.date.add(e, 1, "day")
};
