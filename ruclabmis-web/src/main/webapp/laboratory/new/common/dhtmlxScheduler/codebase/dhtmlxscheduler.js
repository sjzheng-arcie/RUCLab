...
dhtmlx.extend_api = function (a, b, c) {
    ...
    if (d)window[a] = function (a) {
        if (a && typeof a == "object" && !a.tagName) {
            ...
            for (g in a)if (b[g])this[b[g]](a[g]); else g.indexOf("on") == 0 && this.attachEvent(g, a[g])
...
dhtmlxEventable = function (a) {
    ...
    a.eventCatcher = function (a) {
        var c = [], d = function () {
            for (var d = !0, f = 0; f < c.length; f++)if (c[f] != null)var g = c[f].apply(a, arguments), d = d && g;
            return d
        };
...
scheduler.xy = {nav_height: 22, min_event_height: 40, scale_width: 50, bar_height: 20, scroll_width: 18, scale_height: 20, month_scale_height: 20, menu_width: 25, margin_top: 0, margin_left: 0, editor_width: 140, lightbox_additional_height: 50};
...
scheduler.set_actions = function () {
    ...
    this._obj.oncontextmenu = function (a) {
        var b =
            a || event, e = b.target || b.srcElement, f = scheduler.callEvent("onContextMenu", [scheduler._locate_event(e), b]);
        return f
    }
};
    ...
    if (!this.config.readonly) {
        ...
        switch (c) {
            ...
            default:
                ...
                if (e)e.call(this, a); else if (b.parentNode && b != this)return scheduler._on_dbl_click(a, b.parentNode)
        }
    }
};
...
scheduler._on_mouse_move = function (a) {
    if (this._drag_mode) {
        ...
        if (!this._drag_pos || b.force_redraw || this._drag_pos.x != b.x || this._drag_pos.y != b.y) {
            ...
            if (this._drag_mode == "move")c = this._min_date.valueOf() + (b.y * this.config.time_step + b.x * 1440) * 6E4, !b.custom && this._table_view && (c +=
                this.date.time_part(h.start_date) * 1E3), c = this._correct_shift(c), d = h.end_date.valueOf() - (h.start_date.valueOf() - c); else {
                ...
                } else if (d = this.date.date_part(new Date(h.end_date)).valueOf() + b.y * this.config.time_step * 6E4, this._els.dhx_cal_data[0].style.cursor = "s-resize", this._mode ==
                    "week" || this._mode == "day")d = this._correct_shift(d);
                ...
                } else c = this._drag_start; else d <= c && (d = c + this.config.time_step * 6E4)
            }
            var j = new Date(d - 1), m = new Date(c);
...
scheduler._reset_scale = function () {
    if (this.templates[this._mode + "_date"]) {
        ...
        var j = scheduler.date.add(h, 1, this._mode), m = 7;
        if (!this._table_view) {
        ...
        for (var l = 0; l < m; l++) {
            ...
            if (!this._table_view) {
                var p = document.createElement("DIV"), q =
                    "dhx_scale_holder";
        ...
        this._colsS[m] = this._cols[m -
            1] + this._colsS[m - 1];
        if (this._table_view)this._reset_month_scale(b, h, k); else if (this._reset_hours_scale(b, h, k), c.multi_day) {
            var n = "dhx_multi_day";
            this._els[n] && (this._els[n][0].parentNode.removeChild(this._els[n][0]), this._els[n] = null);
            var r = this._els.dhx_cal_navline[0], s = r.offsetHeight + this._els.dhx_cal_header[0].offsetHeight + 1, u = document.createElement("DIV");
            u.className = n;
...
scheduler._reset_month_scale = function (a, b, c) {
    ...
    var m = 0;
    this._min_date = c;
    for (var o = "<table cellpadding='0' cellspacing='0'>",
             l = [], i = 0; i < f; i++) {
        o += "<tr>";
        for (var p = 0; p < 7; p++) {
            o += "<td";
            ...
            var n = "dhx_month_body", r = "dhx_month_head";
    ...
    week_start: function (a) {
        ...
        switch (c) {
            ...
            case "day":
                ...
                break;
            case "month":
                var e = d.getMonth() + b, f = new Date(d);
                d.setMonth(e);
...
scheduler.locale = {date: {month_full: "1月,2月,3月,4月,5月,6月,7月,8月,9月,10月,11月,12月".split(","), month_short: "1月,2月,3月,4月,5月,6月,7月,8月,9月,10月,11月,12月".split(","), day_full: "周日,周一,周二,周三,周四,周五,周六".split(","), day_short: "周日,周一,周二,周三,周四,周五,周六".split(",")}, labels: {dhx_cal_today_button: "今天", day_tab: "日", week_tab: "周", month_tab: "月", new_event: "新的日程安排", icon_save: "保存", icon_cancel: "取消", icon_details: "详细",
    icon_edit: "编辑", icon_delete: "删除", confirm_closing: "", confirm_deleting: "将要删除该事件，是否确定?", section_description: "描述", section_time: "周期", full_day: "全天", confirm_recurring: "Do you want to edit the whole set of repeated events?", section_recurring: "Repeat event", button_recurring: "Disabled", button_recurring_open: "Enabled", button_edit_series: "Edit series", button_edit_occurrence: "Edit occurrence", agenda_tab: "Agenda", date: "Date", description: "Description",
    year_tab: "Year", week_agenda_tab: "Agenda", grid_tab: "Grid"}};
scheduler.config = {default_date: "%j %M %Y", month_date: "%F %Y", load_date: "%Y-%m-%d", week_date: "%l", day_date: "%D, %F %j", hour_date: "%H:%i", month_day: "%d", xml_date: "%m/%d/%Y %H:%i", api_date: "%d-%m-%Y %H:%i", hour_size_px: 42, time_step: 5, start_on_monday: 1, first_hour: 0, last_hour: 24, readonly: !1, drag_resize: 1, drag_move: 1, drag_create: 1, dblclick_create: 1, edit_on_create: 1, details_on_create: 0, click_form_details: 0, cascade_event_display: !1, cascade_event_count: 4, cascade_event_margin: 30, multi_day_height_limit: 0, drag_lightbox: !0,
    preserve_scroll: !0, select: !0, server_utc: !1, positive_closing: !1, icons_edit: ["icon_save", "icon_cancel"], icons_select: ["icon_details", "icon_edit", "icon_delete"], buttons_left: ["dhx_save_btn", "dhx_cancel_btn"], buttons_right: ["dhx_delete_btn"], lightbox: {sections: [
        {name: "description", height: 200, map_to: "text", type: "textarea", focus: !0},
        {name: "time", height: 72, type: "time", map_to: "auto"}
    ]}, highlight_displayed_event: !0, displayed_event_color: "#ffc5ab", displayed_event_text_color: "#7e2727", left_border: !1};
scheduler.templates = {};
...
scheduler._pre_render_events = function (a, b) {
    ...
    if (this._table_view)if (b)this._colsS.heights = d; else {
        var g = f.firstChild;
        if (g.rows) {
            ...
            if (g.parentNode.offsetHeight < g.parentNode.scrollHeight && !g._h_fix) {
                for (h = 0; h < g.rows.length; h++) {
                    var j = g.rows[h].cells[6].childNodes[0], m = j.offsetWidth - scheduler.xy.scroll_width + "px";
                    j.style.width = m;
            ...
            var o = g.parentNode.childNodes, l = (e[0] + 1) * c + 1, p = l, q = l + "px";
            this.config.multi_day_height_limit &&
            (p = Math.min(l, this.config.multi_day_height_limit), q = p + "px");
    ...
    for (var e = 0; e < a.length; e++) {
        ...
        if (!b) {
            ...
            for (var j = c[f._sday]; j.length;) {
                ...
                if (o.valueOf() <= f.start_date.valueOf())j.splice(j.length - 1, 1); else break
            }
...
scheduler._pre_render_events_table = function (a, b) {
    ...
    for (var c = [], d = [
        ...
        var m = h[j], o = f || i.start_date, l = i.end_date;
        ...
        if (i._sday + i._length <= g)f = null, c.push(i), n[r] = i, e[i._sweek] = n.length - 1, i._first_chunk = m.first_chunk, i._last_chunk = m.last_chunk; else {
    ...
    if (!(a._sday < 0)) {
        ...
        if (d) {
            var e = a.start_date.getHours() * 60 + a.start_date.getMinutes(), f = a.end_date.getHours() * 60 + a.end_date.getMinutes() || scheduler.config.last_hour * 60, g = a._count || 1, h = a._sorder || 0, k = Math.round((e * 6E4 - this.config.first_hour * 36E5) * this.config.hour_size_px / 36E5) % (this.config.hour_size_px * 24), i = Math.max(scheduler.xy.min_event_height, (f - e) * this.config.hour_size_px /
                60), j = Math.floor((d.clientWidth - c) / g), m = h * j + 1;
            a._inner || (j *= g - h);
            if (this.config.cascade_event_display)var o = this.config.cascade_event_count, l = this.config.cascade_event_margin, m = h % o * l, p = a._inner ? (g - h - 1) % o * l / 2 : 0, j = Math.floor(d.clientWidth - c - m - p);
            ...
            if (this._edit_id ==
                ...
                if (this._quirks7)n.firstChild.style.height = i -
                    12 + "px";
            ...
            if (this.xy.menu_width !== 0 && this._select_id == a.id) {
                if (this.config.cascade_event_display && this._drag_mode)q.style.zIndex =
                    1;
...
scheduler._render_v_bar = function (a, b, c, d, e, f, g, h, k) {
    var i = document.createElement("DIV"), j = this.getEvent(a), m = k ? "dhx_cal_event dhx_cal_select_menu" : "dhx_cal_event", o = scheduler.templates.event_class(j.start_date, j.end_date, j);
    o && (m = m + " " + o);
    var l = j.color ? "background:" + j.color + ";" : "", p = j.textColor ? "color:" + j.textColor + ";" : "", q = '<div event_id="' + a + '" class="' + m + '" style="position:absolute; top:' + c + "px; left:" + b + "px; width:" + (d - 4) + "px; height:" + e + "px;" + (f || "") + '"></div>';
    i.innerHTML = q;
    var n = i.cloneNode(!0).firstChild;
    if (!scheduler.renderEvent || k || !scheduler.renderEvent(n, j)) {
        var n = i.firstChild, r = '<div class="dhx_event_move dhx_header" style=" width:' + (d - 6) + "px;" + l + '" >&nbsp;</div>';
...
scheduler.render_event_bar = function (a) {
    ...
    var e = this.xy.bar_height, f = this._colsS.heights[a._sweek] + (this._colsS.height ? this.xy.month_scale_height + 2 : 2) + a._sorder * e, g = document.createElement("DIV"), h = "dhx_cal_event_clear";
    ...
    k && (h = h + " " + k);
    var i = a.color ? "background:" + a.color + ";" : "", j = a.textColor ? "color:" + a.textColor + ";" : "", m = '<div event_id="' + a.id + '" class="' + h + '" style="position:absolute; top:' + f + "px; left:" + c + "px; width:" + (d - c - 15) + "px;" + j + "" + i + "" + (a._text_style || "") + '">', a = scheduler.getEvent(a.id);
    ...
    for (var e = a.doXPath("//coll_options"), f = 0; f < e.length; f++) {
        ...
        if (h) {
            ...
            for (var k = a.doXPath(".//item", e[f]), i = 0; i < k.length; i++) {
                for (var j = k[i], m = j.attributes, o = {key: k[i].getAttribute("value"),
                    label: k[i].getAttribute("label")}, l = 0; l < m.length; l++) {
                    var p = m[l];
...
scheduler.form_blocks = {template: {render: function (a) {
    ...
    var b = a.time_format, c = scheduler.config, d = this.date.date_part(new Date), e = 1440, f = 0;
        ...
        switch (k) {
            ...
            case "%H:%i":
                ...
                for (a._time_values =
                    ...
                    var l = d.getDate() != m ? 1 : 0, j = l * 1440 + d.getHours() * 60 + d.getMinutes()
                }
                g += "</select>"
        }
    }
    ...
    if (f.full_day) {
        if (!a._full_day) {
            ...
            (k = a.previousSibling.innerHTML + k);
        ...
        i.onclick = function () {
            if (i.checked) {
                ...
                if (+j == +f || +j >= +f && (c.end_date.getHours() !=
                    0 || c.end_date.getMinutes() != 0))j = scheduler.date.add(j, 1, "day")
            }
            g[h[0]].disabled = i.checked;
            g[h[0] + g.length / 2].disabled = i.checked;
    ...
    this.getLightbox().onclick = function (a) {
        ...
        if (b && b.className)switch (b.className) {
            ...
            case "dhx_cancel_btn":
                ...
                break;
            default:
                if (b.getAttribute("dhx_button"))scheduler.callEvent("onLightboxButton", [b.className, b, a]); else {
                    var d, e, f;
